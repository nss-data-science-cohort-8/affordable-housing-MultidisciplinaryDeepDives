...
```{r}
Price_n_Location <- Price_n_Location |>  
                      add_column(Distance_to_TNA_20130015 = st_distance(Price_n_Location_geo, AffordableHousing_geo))
Price_n_Location
```

```{r}
Price_n_Location <- Price_n_Location |> 
                      rename(Transaction_Price = price, Transaction_Date = ownerdate)
Price_n_Location
```
...
HousesWithin2000ft <- Price_n_Location |> 
                      mutate_at(c('Distance_to_TNA_20130015'), as.numeric) |> 
                      filter(Distance_to_TNA_20130015 <= 609.6) |> 
...
```{r}
HousesWithin2000ft_5to1YrBeforeReopening <- HousesWithin2000ft |> 
                                            filter(Transaction_Date >= '2007-01-01') |> 
                                            filter(Transaction_Date < '2012-01-01') |> 
                                            mutate(AfterReopening_1to5Yr = 0)
HousesWithin2000ft_5to1YrBeforeReopening
```

```{r}
HousesWithin2000ft_1to5YrAfterReopening <- HousesWithin2000ft |> 
                                            filter(Transaction_Date > '2014-12-31') |> 
                                            filter(Transaction_Date <= '2019-12-31') |> 
                                            mutate(AfterReopening_1to5Yr = 1)
HousesWithin2000ft_1to5YrAfterReopening
```
...
HousesBetween2000n3000ft <- Price_n_Location |> 
                      mutate_at(c('Distance_to_TNA_20130015'), as.numeric) |> 
                      filter(Distance_to_TNA_20130015 <= 914.4) |> 
                      filter(Distance_to_TNA_20130015 >= 609.6) |> 
                      mutate(Within2000ft = 0)
HousesBetween2000n3000ft
```
```{r}
HousesBetween2000n3000ft_5to1YrBeforeReopening <- HousesBetween2000n3000ft |> 
                                            filter(Transaction_Date >= '2007-01-01') |> 
                                            filter(Transaction_Date < '2012-01-01') |> 
                                            mutate(AfterReopening_1to5Yr = 0)
HousesBetween2000n3000ft_5to1YrBeforeReopening
```

```{r}
HousesBetween2000n3000ft_1to5YrAfterReopening <- HousesBetween2000n3000ft |> 
                                            filter(Transaction_Date > '2014-12-31') |> 
                                            filter(Transaction_Date <= '2019-12-31') |> 
                                            mutate(AfterReopening_1to5Yr = 1)
HousesBetween2000n3000ft_1to5YrAfterReopening
```


```{r}
BinaryVarsAssignedTibble <- bind_rows(HousesWithin2000ft_5to1YrBeforeReopening, HousesWithin2000ft_1to5YrAfterReopening, HousesBetween2000n3000ft_5to1YrBeforeReopening, HousesBetween2000n3000ft_1to5YrAfterReopening)

BinaryVarsAssignedTibble
```


Difference-in-Difference analysis of prices of houses within 2000ft vs. of houses between 2000ft and 3000ft from affordable housing after its grand reopening did not yield statistically significant result. 
 
```{r}

lr_HousingPrice_DiffinDiff_nearAffordableApt <- glm(Transaction_Price ~ Within2000ft + AfterReopening_1to5Yr + Within2000ft:AfterReopening_1to5Yr, family = gaussian(link = "identity"), data = BinaryVarsAssignedTibble)
